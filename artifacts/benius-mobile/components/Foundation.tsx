...
export function Screen({ children, scroll = true }: { children: React.ReactNode; scroll?: boolean }) {
  ...
  return <View style={{ flex: 1, backgroundColor: c.background }}>
    <OfflineBanner />
    {scroll ? <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={[{ flexGrow: 1 }, padding]}>{children}</ScrollView>
...
export function Field({ label, value, onChangeText, secureTextEntry, disabled }: { label: string; value: string; onChangeText: (v: string) => void; secureTextEntry?: boolean; disabled?: boolean }) {
  const c = useColors();
  return <View style={{ gap: 6 }}><Text style={{ color: c.foreground, fontSize: 14 }}>{label}</Text><TextInput testID={`field-${label.toLowerCase()}`} accessibilityLabel={label} editable={!disabled} autoCapitalize="none" autoCorrect={false} secureTextEntry={secureTextEntry} value={value} onChangeText={onChangeText} placeholder={label} placeholderTextColor={c.mutedForeground} style={[styles.input, { color: c.foreground, borderColor: c.input, backgroundColor: c.card }]} /></View>;
}
