...
export async function sendPasswordRecoveryEmail(
  ...
  if (
    provider === "mailtrap"
    && (typeof config.mailtrapInboxId !== "string" || config.mailtrapInboxId.trim().length === 0)
  ) {
    throw new Error("Mailtrap inbox is not configured");
  }
  ...
  try {
    ...
    const response = provider === "mailtrap"
      ? await fetch(`https://sandbox.api.mailtrap.io/api/send/${config.mailtrapInboxId!.trim()}`, {
          method: "POST",
