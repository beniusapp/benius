...
export default function ClassworkModule({ teacher }: { teacher: TeacherMe }) {
  ...
  const { data: entries = [], isLoading } = useQuery<ClassworkEntry[]>({
    queryKey: ["/api/classwork", teacher.schoolId, teacher.assignedClass, teacher.assignedSection],
    queryFn: async () => {
      const res = await fetch(`/api/classwork/${teacher.schoolId}/${teacher.assignedClass}/${teacher.assignedSection}`, { credentials: "include" });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "Classwork Posted" });
      setContent("");
      if (fileRef.current) fileRef.current.value = "";
      queryClient.invalidateQueries({ queryKey: ["/api/classwork", teacher.schoolId, teacher.assignedClass, teacher.assignedSection] });
    },
    onError: (error: Error) => {
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader>
          <CardTitle className="text-lg" data-testid="text-classwork-title">
            Classwork - Class {teacher.assignedClass} {teacher.assignedSection}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <input type="file" ref={fileRef} className="text-sm" data-testid="input-classwork-file" />
            </div>
            <Button
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Previous Classwork</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <div className="flex justify-center py-6"><Loader2 className="w-5 h-5 animate-spin" /></div>
          ) : entries.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4" data-testid="text-no-classwork">No classwork posted yet.</p>
          ) : (
            <div className="space-y-3">
              {entries.map((e) => (
                <div key={e.id} className="p-3 rounded-md border" data-testid={`card-classwork-${e.id}`}>
