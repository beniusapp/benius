...
export default function StudentArchivesPage() {
  ...
  const handlePrint = () => {
    ...
    const authoritativePercentage = academicResult?.termAverages[selectedExamType] ?? null;
    const authoritativeGrade = academicResult?.termGrades[selectedExamType]?.label ?? null;
    const summary = {
      ...scoresData.summary,
                              ...
                              <tr key={i} style={{ borderBottom: "1px solid #f8fafc" }}>
                                ...
                                <td className="py-3 pr-4">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-xs font-bold text-slate-600">{pct === null ? "Incomplete" : `${pct}%`}</span>
                                  </div>
                                </td>
                                <td className="py-3">
                                  <span className="px-2 py-0.5 rounded-full text-xs font-bold" style={{ background: `${gradeColor(g ?? "")}18`, color: gradeColor(g ?? "") }}>{g ?? "Incomplete"}</span>
                                </td>
                              </tr>
                            );
                          })}
                        ...
                        <>
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            {[
                              ...
                              { label: "Percentage",  value: academicResult?.termAverages[selectedExamType] == null ? "Incomplete" : `${academicResult.termAverages[selectedExamType]}%`, color: "#3b82f6" },
                              { label: "Grade",       value: academicResult?.termGrades[selectedExamType]?.label ?? "Incomplete", color: gradeColor(academicResult?.termGrades[selectedExamType]?.label ?? "") },
                            ].map(stat => (
