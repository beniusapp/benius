...
export default function AttendanceOverview({ schoolId, onViewStudent }: Props) {
  ...
  // School config — admin-scoped endpoint, no hardcoded fallbacks
  ...
  // Class stats derived from student data
  ...
  // Filtered student rows (search by name, roll no, or DSID)
  ...
  // Student detail query for the Quick Action modal
  ...
  // Today's attendance status for the selected student
  ...
  return (
    <div className="space-y-5">
      ...
      {/* School-Wide Daily Summary Strip */}
      <div className="rounded-xl border border-white/10 bg-[#0A1628] p-4">
        ...
        {summaryLoading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            <StatCard label="Marked Today" value={summary?.total ?? 0} color="text-blue-400" bg="bg-blue-500/20" icon={Users} />
      ...
      {filterClass && filterSection && (
        <div className="space-y-4">
          {/* 3 stat cards: Strength | Present / Absent | Attendance % */}
          {studentLoading ? (
      ...
      {/* No-data notice for school-wide */}
      {!summaryLoading && (summary?.total ?? 0) === 0 && (
        <div className="rounded-xl border border-yellow-500/30 bg-yellow-500/10 p-4 text-center">
          <p className="text-yellow-400 text-sm">
            No attendance records found for {displayDate}. Teachers may not have marked attendance yet.
          </p>
        </div>
      )}

      {/* Student Detail Modal — Quick Action per-student drill-down */}
