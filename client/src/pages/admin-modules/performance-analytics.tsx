...
import {
  Loader2, Award, Search, X, FileText, Printer, TrendingUp,
...
// ── Types (identical to teacher examination.tsx) ───────────────────────────
...
function computeAllStudentResults(
  ...
  return students.map(student => {
    ...
    return {
      ...
      promoted, promotionReason, detentionViolations: violations,
    };
  });
}

// ── Grade helpers (exact copy) ─────────────────────────────────────────────
...
// ── Detention reason builder (exact copy) ─────────────────────────────────
              ...
              <div className="rounded-xl p-4 border border-amber-500/20 bg-amber-500/5 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 mt-0.5 leading-relaxed">
                    The Promotion Ledger has not been filled for this student yet. The class teacher needs to run Auto-Suggestion or set decisions manually, then save the ledger.
                  </p>
                </div>
              </div>
            )
...
// ── Read-only Promotion Cell (admin always read-only) ─────────────────────
function PromoCellReadOnly({ entry }: { entry: PromoEntry | undefined }) {
  const decision = entry?.decision ?? null;
  if (!decision) {
    return (
      <span className="inline-flex items-center px-2.5 py-1.5 rounded-full text-xs font-bold border border-slate-700/50 bg-slate-800/30 text-slate-500">
        Not set
      </span>
    );
  }
...
export default function PerformanceAnalytics({ classes, sections: allSections, classSections }: Props) {
  ...
  const resSections = useMemo(() => {
    if (!resClass) return [];
    return (classSections[resClass]?.length > 0) ? classSections[resClass] : allSections;
  }, [resClass, classSections, allSections]);
  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (!r.ok) {
        ...
        throw new Error((body as any).message || `No policy for class ${resClass}`);
      }
      return r.json();
    },
    enabled: !!resClass,
  ...
  const { showCol, cumulConfig } = useMemo(() => {
    const defaults = {
      ...
      cumulConfig: null as CumulConfigShape,
    };
    ...
    <div className="space-y-5" data-testid="tab-results">
      ...
      <div className="rounded-2xl border border-[#1e293b] bg-[#0f172a] p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="ml-auto px-2.5 py-1 rounded-full bg-[#D4AF37]/10 border border-[#D4AF37]/20 text-[#D4AF37] text-[10px] font-bold uppercase tracking-wide">
          ...
          <div className="mt-3 flex flex-col gap-2 rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-400">
            <div className="flex items-start gap-2">
              ...
              <span>{policyError.toLowerCase().startsWith("no policy") || policyError.toLowerCase().startsWith("no exam policy")
                ? `No Exam Policy configured for Class ${resClass}. Set one up in School Setup → Exam Aggregation & Promotion Policy.`
                : policyError
              }</span>
            </div>
            <button
        ...
        <>
          {filteredResults.length === 0 ? (
            ...
            <div className="rounded-2xl border border-[#1e293b] bg-[#0f172a] overflow-hidden" data-testid="results-table">
              ...
              {/* Ledger status bar — read-only for admin */}
              {showCol.promotionGate && (
                <div className="px-4 py-3 border-b border-[#1e293b] flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    {!promoLocked && savedDecisions.length > 0 && (
                    ...
                    <span className="text-[11px] text-slate-500 italic">
                      Promotion decisions are set by the class teacher — Admin view is read-only.
                    </span>
                  </div>
                </div>
              )}
                    ...
                    {filteredResults.map((student, idx) => {
                      ...
                      return (
                        <tr key={student.studentId} className="border-b border-[#1e293b]/60 hover:bg-[#1e293b]/30 transition-colors" data-testid={`result-row-${student.studentId}`}>
                          ...
                          {showCol.promotionGate && (
                            <td className="py-3 px-4 text-center">
                              <PromoCellReadOnly entry={promoMap[student.studentId]} />
                            </td>
                          )}
