...
describe("password recovery email sender", () => {
  ...
  it.each([
    ["missing", undefined],
    ["empty", ""],
  ])("rejects a %s provider without defaulting to SendGrid", async (_label, provider) => {
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);

    await expect(sendPasswordRecoveryEmail(
      baseConfig({ emailProvider: provider as any }),
      "recovery@example.test",
      "123456",
    )).rejects.toThrow("Unsupported password recovery email provider");
    expect(fetchMock).not.toHaveBeenCalled();
  ...
  it.each([
    ...
    ["empty", ""],
    ["whitespace", "   "],
  ])("rejects Mailtrap with a %s inbox without using a default inbox", async (_label, inboxId) => {
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    ...
    await expect(sendPasswordRecoveryEmail(
      baseConfig({
        emailProvider: "mailtrap",
        mailtrapApiKey: "mailtrap-test-key",
        mailtrapInboxId: inboxId as any,
      }),
      "recovery@example.test",
      "123456",
    )).rejects.toThrow("Mailtrap inbox is not configured");
    expect(fetchMock).not.toHaveBeenCalled();
  });
