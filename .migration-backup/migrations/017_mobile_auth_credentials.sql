-- Additive native-mobile authentication storage. Existing web session and
-- principal tables are deliberately untouched.
...
CREATE INDEX mobile_auth_sessions_principal_idx
  ON mobile_auth_sessions(role, principal_id, school_id);
CREATE UNIQUE INDEX mobile_auth_sessions_device_id_uidx
  ON mobile_auth_sessions(device_id);

CREATE TABLE mobile_auth_refresh_tokens (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX mobile_auth_refresh_session_idx
  ON mobile_auth_refresh_tokens(session_id, expires_at);
CREATE INDEX mobile_auth_refresh_family_idx
...
CREATE INDEX mobile_auth_challenges_active_idx
