...
export function renderInvoiceDocument(data: InvoiceDocumentData): string {
  ...
  ${data.notes ? `<section class="policy"><h2>Notes</h2><p>${esc(data.notes)}</p></section>` : ""}<footer class="footer">This document is an invoice and confirms the amount due. A payment receipt is issued separately after successful payment.<br>Computer-generated invoice. No signature is required.</footer></article><script>window.print()</script></body></html>`;
}