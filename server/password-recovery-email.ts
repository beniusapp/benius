...
export async function sendStudentRecoveryContactVerificationEmail(
  config: NotificationConfig,
  recipient: string,
  otp: string,
): Promise<void> {
  if (!config.emailEnabled) throw new Error("Student recovery contact verification email is disabled");
  const provider = config.emailProvider || "sendgrid";
  if (provider !== "sendgrid" && provider !== "mailtrap") {
    throw new Error("Unsupported student recovery contact verification email provider");
  }
  const apiKey = provider === "mailtrap" ? config.mailtrapApiKey : config.sendgridApiKey;
  const fromEmail = config.sendgridFromEmail;
  if (!apiKey || !fromEmail || !fromEmail.includes("@")) {
    throw new Error("Student recovery contact verification email is not configured");
  }
  if (!recipient || !recipient.includes("@")) throw new Error("Invalid verification recipient");
  if (!/^\d{6}$/.test(otp)) throw new Error("Invalid verification OTP");

  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);
  try {
    const subject = "Student recovery email verification";
    const html = `<p>Your student recovery email verification code is <strong>${escapeHtml(otp)}</strong>.</p><p>This code expires in 10 minutes and can be used once.</p><p>If you did not request this, you can safely ignore this email.</p>`;
    const response = provider === "mailtrap"
      ? await fetch(`https://sandbox.api.mailtrap.io/api/send/${config.mailtrapInboxId || "default"}`, {
          method: "POST",
          headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
    ...
    if (!response.ok) throw new Error(`Student recovery contact verification email failed: ${response.status}`);
  } finally {
    clearTimeout(timeout);
  }
}