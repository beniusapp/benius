...
export async function registerRoutes(
  ...
  app.post("/api/admin/calendar/sync-google", async (req, res) => {
    ...
    const url = `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendarId)}/events?key=${apiKey}&singleEvents=true&maxResults=2500&timeMin=${encodeURIComponent(timeMin)}&timeMax=${encodeURIComponent(timeMax)}&orderBy=startTime`;

    let gcalRes: Response;
    try {
      gcalRes = await fetch(url);
    } catch {
      return res.status(502).json({ message: "Failed to reach Google Calendar API. Check your internet connection." });
    }
    ...
    for (const gEvent of googleEvents) {
      ...
      const colorCode = eventType === "holiday" ? "#ef4444" : eventType === "examination" ? "#8b5cf6" : eventType === "academic" ? "#3b82f6" : "#10b981";

      const startD = new Date(startDateStr + "T00:00:00");
      // For all-day events: Google's end date is exclusive (next day); for datetime events just use start day
      const limit = isAllDay && endDateStr ? new Date(endDateStr + "T00:00:00") : new Date(startD.getTime() + 86400000);
    ...
    res.json({ message: `Synced ${entries.length} events from Google Calendar`, count: entries.length });
  });
