...
(async () => {
  ...
  await pool.query(`
    ...
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'student_verified_contacts_student_tenant_fk'
      ) THEN
        ALTER TABLE student_verified_recovery_contacts
          ADD CONSTRAINT student_verified_contacts_student_tenant_fk
          FOREIGN KEY (student_id, school_id) REFERENCES students(id, school_id) ON DELETE CASCADE;
      END IF;
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'student_recovery_verification_student_tenant_fk'
      ) THEN
        ALTER TABLE student_recovery_contact_verification_challenges
          ADD CONSTRAINT student_recovery_verification_student_tenant_fk
          FOREIGN KEY (student_id, school_id) REFERENCES students(id, school_id) ON DELETE CASCADE;
      END IF;
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'student_recovery_verification_contact_tenant_fk'
      ) THEN
        ALTER TABLE student_recovery_contact_verification_challenges
          ADD CONSTRAINT student_recovery_verification_contact_tenant_fk
          FOREIGN KEY (contact_id, student_id, school_id)
          REFERENCES student_verified_recovery_contacts(id, student_id, school_id) ON DELETE CASCADE;
      END IF;
    END $$;
