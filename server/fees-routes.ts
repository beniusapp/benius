...
export function registerFeesRoutes(app: Express) {
  ...
  // ── Payment Record Receipt — PDF download ─────────────────────────────────
  app.get("/api/admin/fees/payments/:id/receipt/pdf", async (req, res) => {
    if (!adminGuard(req, res)) return;
    const schoolId = req.session.schoolId!;
    const id = parseInt(req.params.id);
    if (isNaN(id)) return res.status(400).json({ message: "Invalid ID" });
    ...
    try {
      ...
      if (payRow.payment_method === "Cash" && payRow.denomination_breakdown) {
        ...
        if (Math.abs(denomTotal - paidAmount) > 0.01) {
          return res.status(400).json({
            message: `Cash denomination total (₹${denomTotal.toLocaleString("en-IN")}) does not match the recorded payment amount (₹${paidAmount.toLocaleString("en-IN")}). Receipt PDF cannot be generated until the denomination record is corrected.`,
          });
        }
      }
      ...
      const attemptRow = (await db.execute(sql`
        SELECT rzp_created_at, rzp_captured_at, payment_method AS rzp_method, card_network
        FROM payment_attempts
        WHERE school_id = ${schoolId}
          AND razorpay_payment_id = ${payRow.razorpay_payment_id ?? ""}
        ORDER BY created_at DESC LIMIT 1
      ...
      const receiptData: ReceiptData = {
        school: {
          ...
          addressLine1: schoolRow?.address_line1 ?? null, addressLine2: schoolRow?.address_line2 ?? null,
          city: schoolRow?.city ?? null, state: schoolRow?.state ?? null, pinCode: schoolRow?.pin_code ?? null,
          phone: schoolRow?.phone ?? null, email: schoolRow?.email ?? null,
          affiliationNumber: schoolRow?.affiliation_number ?? null, gstin: schoolRow?.gstin ?? null,
        },
        student: {
          name: studentRow.name, digitalStudentId: studentRow.digital_student_id,
          rollNumber: studentRow.roll_number ?? null,
          class: studentRow.class, section: studentRow.section,
          guardianName: studentRow.guardian_name ?? null,
          phone: studentRow.phone ?? null, email: studentRow.email ?? null,
        },
        fee: {
          feeType: feeRow?.fee_type ?? payRow.payment_method,
          feeName: feeRow?.fee_name ?? feeRow?.fee_type ?? null,
          invoiceNumber: feeRow?.invoice_number ?? null, academicYear: feeRow?.academic_year ?? null,
          feePeriodStart: feeRow?.fee_period_start ? formatDateOnly(feeRow.fee_period_start) : null,
          feePeriodEnd: feeRow?.fee_period_end ? formatDateOnly(feeRow.fee_period_end) : null,
          dueDate: feeRow?.due_date ? formatDateOnly(String(feeRow.due_date).slice(0, 10)) : null,
          amount: feeRow ? Number(feeRow.amount ?? 0) : Number(payRow.amount ?? 0),
          lateFeeAmount: feeRow ? Number(feeRow.late_fee_amount ?? 0) : Number(payRow.late_fee_paid ?? 0),
          breakdown: Array.isArray(feeRow?.breakdown_snapshot) ? feeRow.breakdown_snapshot : [],
        ...
        payment: {
          receiptNumber: payRow.receipt_number ?? null,
          amount: Number(payRow.amount ?? 0), lateFeePaid: Number(payRow.late_fee_paid ?? 0),
          paymentMethod: payRow.payment_method ?? "Cash",
          receivedDate: payRow.received_date ? String(payRow.received_date).slice(0, 10) : todayInIST(),
          paymentDateTimeIST: formatInstantIST(payRow.created_at),
          cashierNotes: payRow.cashier_notes ?? null,
          razorpayPaymentId: payRow.razorpay_payment_id ?? null,
          razorpayOrderId: payRow.razorpay_order_id ?? null,
          paymentMode: payRow.payment_mode ?? null, bankName: payRow.bank_name ?? null,
          cardLast4: payRow.card_last4 ?? null,
          cardNetwork: attemptRow?.rzp_method === "card" ? (attemptRow?.card_network ?? null) : null,
          vpa: payRow.vpa ?? null, payerName: payRow.payer_name ?? null,
          payerEmail: payRow.payer_email ?? null, payerContact: payRow.payer_contact ?? null,
          gatewayStatus: payRow.gateway_status ?? null,
          providerCreatedIST: attemptRow?.rzp_created_at ? formatInstantIST(attemptRow.rzp_created_at) : null,
          providerCapturedIST: attemptRow?.rzp_captured_at ? formatInstantIST(attemptRow.rzp_captured_at) : null,
          denominationBreakdown: payRow.denomination_breakdown ?? null,
          ...
          branchName: payRow.branch_name ?? null,
          offlineDetail: odRow ? {
            transactionTime: odRow.transaction_time ?? null, instrumentStatus: odRow.instrument_status ?? null,
            transferMode: odRow.transfer_mode ?? null, transactionReference: odRow.transaction_reference ?? null,
            receivingBank: odRow.receiving_bank ?? null, receiverUpiId: odRow.receiver_upi_id ?? null,
            payeeName: odRow.payee_name ?? null, payableAt: odRow.payable_at ?? null,
            collectionLocation: odRow.collection_location ?? null, depositDate: odRow.deposit_date ?? null,
            depositBank: odRow.deposit_bank ?? null, depositReference: odRow.deposit_reference ?? null,
            returnDate: odRow.return_date ?? null, returnReason: odRow.return_reason ?? null,
          } : null,
          recordedByName: payRow.recorded_by_display_name ?? null,
          recordedByRole: payRow.recorded_by_role ?? null,
        },
      ...
      const receiptNo = payRow.receipt_number ?? `pay-${id}`;
      ...
      console.error("[payment receipt pdf]", err);
      res.status(500).json({ message: String(err) });
    }
  });
...
<script>window.print();</script>
  ...
  // ── Fee Record Receipt — PDF download ─────────────────────────────────────
  app.get("/api/admin/fees/:id/receipt/pdf", async (req, res) => {
    if (!adminGuard(req, res)) return;
    const schoolId = req.session.schoolId!;
    const id = parseInt(req.params.id);
    if (isNaN(id)) return res.status(400).json({ message: "Invalid ID" });
    ...
    try {
      ...
      if (!feeRow) return res.status(404).json({ message: "Fee record not found" });
      if (feeRow.status !== "Paid") {
        return res.status(400).json({ message: "Receipt PDF only available for paid records" });
      }
      ...
      const receiptData: ReceiptData = {
        ...
        student: {
          name: feeRow.student_name, digitalStudentId: feeRow.digital_student_id,
          rollNumber: feeRow.roll_number ?? null,
          class: feeRow.student_class, section: feeRow.student_section,
          guardianName: feeRow.guardian_name ?? null, phone: feeRow.student_phone ?? null, email: feeRow.student_email ?? null,
        },
        fee: {
          feeType: feeRow.fee_type, feeName: feeRow.fee_name ?? feeRow.fee_type,
          invoiceNumber: feeRow.invoice_number ?? null, academicYear: feeRow.academic_year ?? null,
          feePeriodStart: feeRow.fee_period_start ? formatDateOnly(String(feeRow.fee_period_start).slice(0, 10)) : null,
          feePeriodEnd: feeRow.fee_period_end ? formatDateOnly(String(feeRow.fee_period_end).slice(0, 10)) : null,
          dueDate: feeRow.due_date ? formatDateOnly(String(feeRow.due_date).slice(0, 10)) : null,
          amount: Number(feeRow.amount ?? 0), lateFeeAmount: Number(feeRow.late_fee_amount ?? 0),
          breakdown: Array.isArray(feeRow.breakdown_snapshot) ? feeRow.breakdown_snapshot : [],
          notes: feeRow.notes ?? null,
        },
        payment: {
          receiptNumber: feeRow.receipt_number ?? payRow?.receipt_number ?? null,
          amount: Number(payRow?.amount ?? feeRow.amount ?? 0), lateFeePaid: Number(payRow?.late_fee_paid ?? 0),
          paymentMethod: payRow?.payment_method ?? "Online",
          receivedDate: payRow?.received_date ? String(payRow.received_date).slice(0, 10) : (feeRow.paid_date ? String(feeRow.paid_date).slice(0, 10) : todayInIST()),
          paymentDateTimeIST: formatInstantIST(paymentInstant),
          cashierNotes: payRow?.cashier_notes ?? null,
          razorpayPaymentId: payRow?.razorpay_payment_id ?? null, razorpayOrderId: payRow?.razorpay_order_id ?? null,
          paymentMode: payRow?.payment_mode ?? null, bankName: payRow?.bank_name ?? null,
          cardLast4: payRow?.card_last4 ?? null, cardNetwork: attemptRow?.card_network ?? null,
          vpa: payRow?.vpa ?? null, payerName: payRow?.payer_name ?? null,
          payerEmail: payRow?.payer_email ?? null, payerContact: payRow?.payer_contact ?? null,
          gatewayStatus: payRow?.gateway_status ?? null,
          providerCreatedIST: attemptRow?.rzp_created_at ? formatInstantIST(attemptRow.rzp_created_at) : null,
      ...
      const receiptNo = feeRow.receipt_number ?? payRow?.receipt_number ?? `fee-${id}`;
      const pdfBuffer = await renderReceiptPdf(receiptData);
      res.setHeader("Content-Type", "application/pdf");
      res.setHeader("Content-Disposition", `attachment; filename="Receipt-${receiptNo}.pdf"`);
      res.send(pdfBuffer);
    } catch (err) {
      console.error("[fee receipt pdf]", err);
      res.status(500).json({ message: String(err) });
    }
  });
